    ...
    <div className="fixed inset-0 z-50">
      ...
      <div className="absolute right-0 top-0 bottom-0 w-full max-w-[480px] bg-white shadow-2xl flex flex-col animate-slide-in">
        ...
        <div className="flex-1 overflow-y-auto px-6 py-5 space-y-4">
          {/* ── Add Mode Tabs (only when adding new, and other recipients exist) ── */}
          {!isEditing && hasOtherRecipients && (
            <div className="flex items-center gap-1 p-1 bg-gray-100 rounded-xl">
              ...
              >
                ...
                Уже добавленный
                <span className="text-[10px] text-gray-400 bg-gray-200 rounded-full px-1.5 py-0.5">{otherRecipients.length}</span>
              </button>
            </div>
          )}

          {/* ── Tab: Existing Recipients from Other Scenarios ── */}
          {!isEditing && addMode === 'existing' && hasOtherRecipients && (
          ...
          {/* ── New Recipient Form (show when: editing, or addMode==='new', or no other recipients) ── */}
          {(isEditing || addMode === 'new' || !hasOtherRecipients) && (
            <>
        ...
        {/* Footer (only when editing or new recipient form is visible) */}
        {!(!isEditing && addMode === 'existing' && hasOtherRecipients) && (
